...
## 🚨 涉及 AI 回答的項目，一次通過不算通過

模型每次回答都可能不同。**跑一次就打勾，等於把「有時會過」當成「修好了」帶進決賽。**
07/31–08/01 這條規則是三次踩坑換來的：

| 實測 | 只跑一次會得到的結論 | 真相 |
|---|---|---|
| F3 RAG 附出處 | 第一次通過 → 「修好了」 | 連跑三次 0/3；出處只在檔案開頭，切塊後多數片段沒有 |
| D9 三方案卡 | 出現三張卡 → 「正常」 | 同一句話有時直接出卡、有時反問「要我產生嗎」，Demo 會卡住 |
| D12 確認卡 | 成交成功 → 「沒問題」 | 連按三次會在成交後跳紅字，看起來像失敗 |

**規則**：D7–D13、D16、F1–F6 這些吃模型回答的項目，**同一項連跑三次全過才算通過**。
自動化的部分直接重跑指令即可：

```bash
for i in 1 2 3; do python3 scripts/verify_live.py --only F; done
```

任一次失敗就是不穩定，要當成失敗處理——**不穩定比全壞更危險**，因為它會在最不該壞的時候壞。
