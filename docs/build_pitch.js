...
const SLIDES = [
  ...
  { type: "cards",
    ...
    foot: "**提示詞攔的是意圖，架構攔的是能力。** 靠提示詞說「不要下單」，是在賭模型每次都聽話。\n\n"
        + "**07/31 真錢實測**：ETH/TWD 兩筆成交（#20720919534、#20721028463），MAX App 推播確認。"
        + "其中一次使用者 1.7 秒內連按三次確認鈕——**只成交一次，後兩次回 410**。"
        + "重放保護不是投影片上的宣稱，是真錢環境下擋下來的。" },
  ...
  { type: "statement",
    ...
    body: "手續費、賣後集中度、機會成本——**全部由確定性程式計算**。\n\n"
        ...
        + "費率不是抄來的：`/api/v3/info` 對這個帳號回傳 **maker 0.0008／taker 0.0016**，"
        + "程式用的就是交易所自己回的值。" },
  ...
  { type: "honest",
    ...
    items: [
      ...
      ["RAG 附出處尚未穩定", "知識庫已建好、檢索正確、出處掛在 metadata 上；但模型是否把出處寫進回答仍有波動，連跑三次不是每次都過。已知不穩定，不是已知損壞。"],
    ],
