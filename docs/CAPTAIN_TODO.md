...
> 更新：2026-08-02（決賽第二天）。**Part 1 與 Part 2 的加分證據都已交齊**（Kiro F–J 由
> chaocongyang 08/02 補完，#42 已關）。剩下的是上台前的收尾。
...
| 6 | **Kiro 證據截圖** | **F–J 五張已交**，存受限 Drive（#42 已關） | 確認 Drive 權限只給指定成員（J 含帳號資訊，不可公開） |
...
| 優先序 | 事項 | 為什麼 |
|---|---|---|
| 1 | Demo 影片收尾（issue #75 的四項待決） | 現場網路出事時的唯一備援 |
| 2 | 上台前對一次簡報數字與線上站 | 數字對不上會在 Q&A 被追 |
| 3 | **不要口頭報 spec 份數** | 交出去的 Specs 面板截圖是 08/02 10:30 拍的（7 個 spec、4 份 steering），之後 repo 又各多一份。報「每個功能一份 spec、三件套齊全」不會對不起來 |
| 4 | Kiro credit 留 ≥700 | 當天還要跑 task（目前用 261.62 / 2000） |

---

## 一句話總結

**加分那 10 分的證據都已經在手上**（Lv2：真實成交兩筆＋權限截圖；Kiro：F–J 五張）。
剩下的都是收尾——影片剪完、簡報數字對過、上台前別把 spec 份數講死。
