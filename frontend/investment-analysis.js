...
function currentAuthorizedData() {
  ...
  return {
    ...
    transactions: scopes.includes("transactions") ? mock.demoTransactions : [],
    funding: scopes.includes("fundingHistory") ? mock.demoFunding : [],
    profile: state.profile || null,
  };
}

function calculateCoverage(data) {
  const tx = data.transactions.slice().sort((a, b) => Date.parse(a.time) - Date.parse(b.time));
  const coverageDays = tx.length > 1 ? daysBetween(tx[0].time, tx[tx.length - 1].time) + 1 : tx.length;
  const missingFields = [];
  if (!data.portfolio) missingFields.push("portfolio");
  if (!tx.length) missingFields.push("transactions");
  if (!data.funding.length) missingFields.push("fundingHistory");
  if (!data.profile || data.profile.status !== "completed") missingFields.push("questionnaire");
  let coverage = "limited";
  if (tx.length >= ANALYSIS_RULES.fullCoverageMinimumTransactions && data.portfolio && data.profile && data.profile.status === "completed") coverage = "full";
  else if (tx.length >= ANALYSIS_RULES.sufficientCoverageMinimumTransactions && data.portfolio) coverage = "sufficient";
  return {
    coverage,
    portfolioAvailable: Boolean(data.portfolio),
    transactionCount: tx.length,
    transactionCoverageDays: coverageDays,
    fundingHistoryAvailable: data.funding.length > 0,
    ...
    questionnaireAnswerCount: data.profile && data.profile.status === "completed"
      ...
      : 0,
    assetCount: data.portfolio ? data.portfolio.assets.length : 0,
    missingFields,
  };
}
...
function calculateAllocation(data) {
  if (!data.portfolio || !data.portfolio.assets.length) {
    return { assetCount: 0, topAssetRatio: null, topTwoAssetsRatio: null, allocationByAsset: [], concentrationLevel: "insufficientData" };
  }
  ...
  const topTwo = allocation.slice(0, 2).reduce((sum, item) => sum + item.ratio, 0);
  return {
    assetCount: allocation.length,
    topAssetRatio: top,
    topTwoAssetsRatio: topTwo,
...
function calculateTrading(data) {
  const tx = data.transactions.slice().sort((a, b) => Date.parse(a.time) - Date.parse(b.time));
  if (!tx.length) return { tradesPerMonth: null, activeTradingDays: 0, medianDaysBetweenTrades: null, buySellRatio: null, clusteredTradeRatio: null, tradingRhythm: "insufficientData" };
  const dates = [...new Set(tx.map((item) => item.time.slice(0, 10)))];
  const gaps = dates.slice(1).map((date, index) => daysBetween(dates[index], date));
  const coverageDays = tx.length > 1 ? daysBetween(tx[0].time, tx[tx.length - 1].time) + 1 : 30;
  const months = Math.max(1, coverageDays / 30.4375);
  const buys = tx.filter((item) => item.side === "buy").length;
  const sells = tx.filter((item) => item.side === "sell").length;
  const clustered = gaps.filter((gap) => gap <= ANALYSIS_RULES.clusteredGapDays).length;
  const tradesPerMonth = round(tx.length / months, 1);
  let tradingRhythm = "occasional";
  if (clustered > gaps.length / 2 && gaps.length) tradingRhythm = "clustered";
  else if (tradesPerMonth >= 8) tradingRhythm = "active";
  else if (tradesPerMonth >= 2) tradingRhythm = "regular";
  return {
    tradesPerMonth,
    activeTradingDays: dates.length,
    medianDaysBetweenTrades: median(gaps),
    buySellRatio: sells ? round(buys / sells, 1) : null,
    clusteredTradeRatio: gaps.length ? round(clustered / gaps.length, 2) : 0,
    tradingRhythm,
  };
}
...
function profileTagsFromMetrics(coverage, allocation, trading, profile) {
  ...
  if (allocation.concentrationLevel === "concentrated") tags.push({ id: "concentrated-allocation", label: "主要持倉較集中", sourceDimension: "allocationPattern" });
...
function fallbackResult(job, data, generatedBy) {
  ...
  const topPct = allocation.topAssetRatio == null ? null : Math.round(allocation.topAssetRatio * 100);
  const topTwoPct = allocation.topTwoAssetsRatio == null ? null : Math.round(allocation.topTwoAssetsRatio * 100);
  ...
  const dimensions = [
    {
      ...
      descriptor: !coverage.transactionCount
        ? "還需要更多資料"
        : trading.tradingRhythm === "occasional" ? "偶爾調整，交易間隔較長" : "依目前紀錄整理出的交易節奏",
      ...
      evidence: coverage.transactionCount ? [
        { id: "trades-per-month", label: "平均每月交易", value: trading.tradesPerMonth == null ? "資料不足" : "約 " + trading.tradesPerMonth + " 次", source: "transactions" },
        { id: "active-days", label: "有交易的日期", value: trading.activeTradingDays + " 天", source: "transactions" },
    ...
    },
    {
      ...
      summary: topPct == null ? "目前持倉資料不足。" : "目前帳戶的變化，較容易受到主要持倉影響。",
      explanation: topPct == null
        ? "目前沒有足夠持倉資料可整理。"
        : "目前帳戶的變化較容易受到主要持倉影響。這只是配置現況，不代表好壞或投資能力。",
      evidence: topPct == null ? [] : [
        { id: "top-asset", label: "最大持倉占比", value: "約 " + topPct + "%", source: "portfolio" },
  ...
  const summary = [
    tradingDataSufficient
      ? "目前 " + coverage.transactionCount + " 筆交易分布在 " + trading.activeTradingDays + " 個日期，操作並不頻繁；"
      : coverage.transactionCount
  ...
  if (coverage.portfolioAvailable) {
    stablePatterns.push({ id: "stable-asset-count", text: "目前持有 " + coverage.assetCount + " 項資產。", evidenceIds: ["asset-count"], sourceLabels: ["持倉資料"] });
  }
  if (coverage.transactionCount) {
    stablePatterns.push({ id: "stable-active-days", text: "可用交易分布在 " + trading.activeTradingDays + " 個日期。", evidenceIds: ["active-days"], sourceLabels: ["交易紀錄"] });
  }
