...
涵蓋 C 段端點、D 段靠對話的項目、F 段安全紅線；**實際判定幾項以跑完印出的統計列為準**
（D 段會依對話走向與 --slow 增減，寫死數字必定過期）。
剩下要用眼睛看的（圓環是不是圓的、行情閃不閃、離線切換）留給 verify_live_ui.js
與人工，本程式會明列是哪幾項，不會假裝測過。

⚠ 已知涵蓋缺口：**沒有任何一項會觸發深度意圖**（「為什麼／分析／歸因／比較」會路由到
Sonnet）。08-01 因此讓一個「深度問題一律 500」的 modelId 錯誤全程綠燈溜過去。補一項進來。
